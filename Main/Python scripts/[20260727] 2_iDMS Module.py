...
meanAllsigma = pd.read_csv(os.path.join(M1_Output, 'meanAllsigma.csv'), index_col = 0)
Intersected_gauss = pd.read_csv(os.path.join(M1_Output, 'Intersected gauss.csv'), index_col = 0)
CombinedAnalysis = pd.read_csv(os.path.join(M1_Output, 'CombinedAnalysis.csv'), index_col = 0)
...
TrainingSet.to_csv('Module2_Outputs/TrainingSet.csv')
...
for i in range(len(Predlist_Lipids)):
    ...
    for SV in listSV:
        ...
        if pred_intersect is None:
            ...
            y_at_intersect = gaussian(pred_intersect, mu_isomer1, sig_isomer1)
...
pred_Intersected_gauss.to_csv('Module2_Outputs/PredictionResult.csv')
